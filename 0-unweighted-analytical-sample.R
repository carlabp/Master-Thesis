...
pns_df <- get_pns(year=2019, vars=c( "V001", "V0026", "C004","C006","C008",
                                     ...
                                     "Q092", "F007011", "VDE014", "VDF002", "VDC001"), design=F)
...
children_df <- filtered_df %>% 
  filter(C004 %in% c(  "Filho(a) somente do responsável",
                       "Filho(a) somente do cônjuge",
                       "Filho(a) do responsável e do cônjuge"))



#List the distinct ID_DOMICILIOS of families with children younger than 18 
...
families_w_children <- families_w_children %>%
  mutate(young_child_indicator = ifelse(C008 <= 3, 1, 0)) %>% 
